...
SMIPath = "..\03_DOC\04_SMI\CI_R8MV_SMI_Left.xlsx";
...
HeaderTable = readtable(SMIPath, 'Sheet', 'Header', 'ReadVariableNames', false, 'Format', 'auto');
InputsTable = readtable(SMIPath, 'Sheet', 'Inputs', 'Format', 'auto');
OutputsTable = readtable(SMIPath, 'Sheet', 'Outputs', 'Format', 'auto');
RunnablesTable = readtable(SMIPath, 'Sheet', 'Runnables', 'Format', 'auto');
PublicDataTable = readtable(SMIPath, 'Sheet', 'PublicData', 'Format', 'auto');
PrivateDataTable = readtable(SMIPath, 'Sheet', 'PrivateData', 'Format', 'auto');
PrivateSignalsTable = readtable(SMIPath, 'Sheet', 'PrivateSignals', 'Format', 'auto');
NumericTypesTable = readtable(SMIPath, 'Sheet', 'NumericTypes', 'Format', 'auto');
BusTable = readtable(SMIPath, 'Sheet', 'Bus', 'Format', 'auto');
EnumTable = readtable(SMIPath, 'Sheet', 'Enum', 'Format', 'auto');
...
for row = 1:num_rows
    ...
    if (~isempty(signalName))
        ...
        if(~contains(signalDataType,'Enum:') && ~contains(signalDataType,'Bus:'))
...
for row = 1:num_rows
    ...
    if (~isempty(paramName))
        ...
        if(~contains(paramDataType,'Enum:') && ~contains(paramDataType,'Bus:'))
...
for row = 1:num_rows
    ...
    if (~isempty(paramName))
        ...
        if(~contains(paramDataType,'Enum:') && ~contains(paramDataType,'Bus:') && ~contains(paramDataType,'double'))
            evalin('base',[paramName,'.Min = paramMin;']);
        end
