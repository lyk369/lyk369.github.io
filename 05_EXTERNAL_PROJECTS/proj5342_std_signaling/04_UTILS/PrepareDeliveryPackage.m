...
% Handle unit tests
copyfile('./02_TEST/02_UNIT_TESTS/01_TEST_CASES',strcat(packageDir,'/02_TEST/02_UNIT_TESTS/01_TEST_CASES'),'f');

filelist = dir(fullfile('./02_TEST/02_UNIT_TESTS/02_MIL/Sentinel Output', '**\*.*'));
...
filelist = dir(fullfile('./02_TEST/03_INTEGRATION_TESTS/03_SIL/Sentinel Output', '**\*.*'));
numel = size(filelist,1);
for x = 1:numel
    if(strcmp(filelist(x).name, 'Report') && (filelist(x).isdir == 1))
        tempPath = extractAfter(strcat(filelist(x).folder,'/',filelist(x).name), '03_SIL\');
