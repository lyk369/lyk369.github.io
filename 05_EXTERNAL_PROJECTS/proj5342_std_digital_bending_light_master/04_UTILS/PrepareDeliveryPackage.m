...
% Handle Test Folder
copyfile('./02_TEST/01_TEST_CASES',strcat(packageDir,'/02_TEST/01_TEST_CASES'),'f');
copyfile('./02_TEST/02_STATIC/01_VMAAC',strcat(packageDir,'/02_TEST/02_STATIC/01_VMAAC'),'f');
copyfile('./02_TEST/02_STATIC/02_KLOCWORK',strcat(packageDir,'/02_TEST/02_STATIC/02_KLOCWORK'),'f');

filelist = dir(fullfile('./02_TEST/03_MIL/Sentinel_Output', '**\*.*'));
...
for x = 1:numel
    if(strcmp(filelist(x).name, 'Report') && (filelist(x).isdir == 1))
        ...
        copyfile(filelist(x).folder,strcat(packageDir,'/02_TEST/03_MIL/',tempPath),'f');
    end
end

filelist = dir(fullfile('./02_TEST/04_SIL/Sentinel_Output', '**\*.*'));
numel = size(filelist,1);
for x = 1:numel
    if(strcmp(filelist(x).name, 'Report') && (filelist(x).isdir == 1))
        disp(filelist(x).folder);
        tempPath = extractAfter(strcat(filelist(x).folder,'/',filelist(x).name), '04_SIL\');
        copyfile(strcat(filelist(x).folder,'/',filelist(x).name),strcat(packageDir,'/02_TEST/04_SIL/',tempPath),'f');
    end
end

filelist = dir(fullfile('./02_TEST/05_PIL/Sentinel_Output', '**\*.*'));
numel = size(filelist,1);
for x = 1:numel
    if(strcmp(filelist(x).name, 'Report') && (filelist(x).isdir == 1))
        disp(filelist(x).folder);
        tempPath = extractAfter(strcat(filelist(x).folder,'/',filelist(x).name), '05_PIL\');
        copyfile(strcat(filelist(x).folder,'/',filelist(x).name),strcat(packageDir,'/02_TEST/05_PIL/',tempPath),'f');
    end
end
